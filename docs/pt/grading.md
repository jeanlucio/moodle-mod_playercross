...
recompensa tanto a resolução cuidadosa dos termos quanto o palpite direto e confiante na
...
* A nota configurada da atividade é dividida igualmente entre os termos da rodada: `nota ÷
  num_terms` é o teto que um único termo resolvido pode valer.
* **Resolver um termo** ganha crédito total nas duas primeiras tentativas — um segundo palpite
  confiante não é tratado como menos merecedor que um de primeira tentativa — depois decresce
  linearmente conforme o `Máximo de tentativas por termo` se aproxima. Com tentativas ilimitadas
  por termo (0), um termo resolvido sempre ganha crédito total, já que não há um denominador
  natural para escalar.
* **Acertar a frase-mistério diretamente** ganha um bônus inversamente proporcional a quantos
  termos já foram resolvidos naquele momento: acertar antes de resolver qualquer termo vale tanto
  quanto todos os termos restantes valeriam em crédito total cada um, recompensando a dedução
  precoce; acertar depois de já resolver todos os termos não ganha bônus nenhum, já que o crédito
  total já foi coletado dos próprios termos.
* O total que uma rodada pode ganhar — pontos dos termos resolvidos mais o bônus da frase-mistério
...
qualquer estudante, `Termos por rodada` (`num_terms`) e `Método de avaliação` travam ambos — da
mesma forma que o Moodle já trava o campo "Nota máxima" de uma atividade avaliada assim que notas
reais existem. Como o denominador de pontos por termo depende diretamente de `num_terms`, mudá-lo
...
termos resolvidos, tentativas usadas, tempo e pontuação — pela página de histórico da barra de

...
toggle) — points are computed by a single fixed formula that rewards both careful term-solving
...
* The activity's configured grade is split evenly across the round's terms: `grade ÷ num_terms`
  is the ceiling a single resolved term can ever be worth.
* **Resolving a term** earns full credit on the first two attempts — a confident second guess is
  not treated as less deserving than a first-try one — then scales down linearly as `Maximum
  attempts per term` is approached. With unlimited attempts per term (0), a resolved term always
...
* **Guessing the mystery phrase directly** earns a bonus inversely proportional to how many
  terms were already resolved at that moment: guessing before solving anything is worth as much
  as every remaining term would have been at full credit each, rewarding early deduction;
  guessing after every term is already solved earns no bonus, since full credit was already
  collected from the terms themselves.
* The total a round can ever earn — resolved-term points plus the final-guess bonus — is always
...
round` (`num_terms`) and `Grading method` both lock — the same way Moodle already locks a
graded activity's own "Maximum grade" field once real grades exist. Since the score-per-term
denominator depends directly on `num_terms`, changing it after real scores exist would make

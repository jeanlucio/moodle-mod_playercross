...
class words_repository {
    ...
    /**
     * @var int Max length of playercross_words.word (db/install.xml char(100)). Public:
    ...
    public static function sync_glossary_words(\stdClass $instance): int {
        ...
        foreach ($entries as $entry) {
            ...
            foreach ($words as $word) {
                ...
                // playercross_add_instance()/update_instance() call this on every
        ...
        if (!empty($orphanids)) {
            ...
            $DB->delete_records_select('playercross_words', "id $delsql AND playercrossid = :instanceid", $delparams);
        }
    ...
    public static function get_recent_words(
        ...
        $total = $DB->count_records('playercross_words', ['playercrossid' => $instanceid]);
        ...
        $sql = "SELECT w.id, w.word, w.source, w.approved, w.concept, g.name AS glossaryname"
            ...
            . " ORDER BY w.$sort $dir";
        $rows = $DB->get_records_sql($sql, ['playercrossid' => $instanceid], $page * $perpage, $perpage);

...
class round_presenter {
    ...
    public static function ranking_scoring_explanation(\stdClass $instance): string {
        ...
        $mode = (int)($instance->rankingscoringmode ?? PLAYERCROSS_SCORING_BINARY);
        $detailkey = $mode === PLAYERCROSS_SCORING_LINEAR
            ? 'ranking_scoringexplanation_linear'
            : 'ranking_scoringexplanation_binary';

        return get_string('ranking_scoringexplanation', 'mod_playercross', get_string($detailkey, 'mod_playercross'));
    }
     ...
     * announce — and always blank for the guest account, which finish_round() never
     * actually grants anything to.
     *
    ...
    private static function build_hud_reward_label(\stdClass $instance, array $state): string {
        ...
        if ($rewarditem <= 0 || empty($state['won']) || isguestuser()) {
            return '';
        }
    ...
    public static function build_lobby_context(\stdClass $instance, array $state, int $userid): array {
        ...
        if ($roundcostitem > 0 && !isguestuser()) {

...
function xmldb_playercross_upgrade(int $oldversion): bool {
    ...
    if ($oldversion < 2026072300) {
        // Add theme_max_length, defaulting to 0 (unlimited) for every activity, new or
        // already existing — the plugin never enforced any ceiling before this, so a
        // default of 0 changes nothing for anyone already using it.
        $table = new xmldb_table('playercross');
        $field = new xmldb_field('theme_max_length', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    ...
    if ($oldversion < 2026081600) {
        ...
        upgrade_mod_savepoint(true, 2026081600, 'playercross');
    }
